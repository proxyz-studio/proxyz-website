...
const COOKIE_PREFIX = 'meetings_unlocked';
const COOKIE_PAYLOAD = 'v1';

/** Slugs that own a meetings vault. Mirrors the partner pages that mount
 *  the <PartnerMeetings> widget. Per-partner code lives in
 *  MEETINGS_CODE_<UPPER_SNAKE_CASE_SLUG>; a shared fallback lives in
 *  MEETINGS_CODE so v1 partners share a single passcode. */
const ALLOWED_SLUGS = new Set(['fast-fix', 'lazy-tiger']);

function slugToEnvKey(slug: string) {
  return `MEETINGS_CODE_${slug.toUpperCase().replace(/-/g, '_')}`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const sharedCode = process.env.MEETINGS_CODE;
  const expectedCode = perPartnerCode || sharedCode;
  ...
  res.setHeader(
    'Set-Cookie',
    `${cookieName}=${token}; HttpOnly; Secure; Path=/; Max-Age=${thirtyDays}; SameSite=Lax`
  );
