...
export default function MediaGate({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/media-check?slug=${encodeURIComponent(slug)}`, { credentials: 'same-origin' })
      .then((r) => r.json())
      .then((d) => {
  ...
  async function submit(value: string) {
    ...
    setError(null);
    try {
      const r = await fetch('/api/media-auth', {
        method: 'POST',
  ...
  return (
    ...
    >
      <div style={{ maxWidth: '440px', width: '100%' }}>
        ...
        >
          {projectName} is a PROXYZ media project in development. Use the
          4-digit code from your invite to view the project.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          <input
            ...
            maxLength={4}
            disabled={submitting}
            aria-label="Four-digit project code"
            placeholder="0000"
            style={{
              fontFamily: FONT_MONO,
              fontSize: '32px',
        ...
        >
          ...
          <a
            href={`mailto:hello@proxyz.studio?subject=${encodeURIComponent(
              `${projectName} access`
            )}`}
            style={{
              color: 'rgba(255,255,255,0.55)',
