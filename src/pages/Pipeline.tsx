...
function PartnerEntry({ partner, index }: { partner: PartnerCard; index: number }) {
  ...
  const bodyId = `partner-body-${partner.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
  ...
  return (
    ...
    >
      ...
      {/* Toggle header — always visible, click to expand/collapse */}
      <button
        type="button"
      ...
      >
        ...
        {/* Subtitle: sector · location (always visible for scannability) */}
        <div
          style={{
      ...
      {expanded && (
        <div
          ...
          style={{
            marginTop: '36px',
            animation: 'partnerBodyIn 180ms ease forwards',
          }}
        >
          {/* Meta row */}
          <div
      ...
      <main style={{ background: '#0A0A0A', color: '#F2F2F2' }}>
        ...
        >
          ...
          <div className="pipeline-hero-inner" style={{ position: 'relative', zIndex: 2, maxWidth: '1360px', margin: '0 auto' }}>
            ...
            */}
            ...
            >
              {c.partners.filter((p) => !p.hidden).length} companies · active discussion
            </p>
            </Reveal>
          </div>
        </section>
        ...
        <section className="section-mobile" style={{ padding: '40px 40px 120px' }}>
          <div style={{ maxWidth: '1360px', margin: '0 auto' }}>
            {c.partners.filter((p) => !p.hidden).map((p, i) => (
